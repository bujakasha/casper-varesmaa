...
import {generateDates} from '../../../utils/forms'
...
class SoittopyyntoForm extends React.Component {
  ...
  sendForm = event => {
    this.setState({loading: true}, () => {
      postForm(this.state, 'Soittopyyntö').then(res => {
        if (res.err) {
          this.setState({
  ...
  render() {
    const {ajankohta, email, tunti, nimi, puhelin, errors, loading} = this.state
    return (
      <form
        className="lomake"
